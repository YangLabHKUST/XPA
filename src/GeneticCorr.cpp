...
void GeneticCorr::normalizeSingleSnp(uchar *genoLine, double *normalizedSnp, const double* maskIndivs, uint64 numSamples, uint64 numUsed) {
...
void GeneticCorr::computeSinglePosteriorMean(const vector<string> &bimFiles, const vector<string> &bedFiles,
  ...
  for (uint i = 0; i < bedFiles.size(); i++) {
    ...
    while (getline(finBim, line)) {
      ...
      normalizeSingleSnp(genoLine, normalizedSnp, maskIndivs, numPad, numUsed);
